...
def main():
    ...
    def spawn_boss():
        global boss
        inimigos.empty()
        boss_img = pygame.transform.scale(pygame.image.load(os.path.join("PIXEL_ART", "chefe.png")), (120, 120))  
        boss_width = boss_img.get_width()  
        boss_height = boss_img.get_height() 
        boss_x = (largura - boss_width) // 2 
        boss_y = -boss_height
        boss = Inimigo(boss_x, boss_y, boss_img, hp=100, vel_y=vel_y_chefe)  # Chefe herda as variáveis da 'class' Inimigo
    ...
    def display_redraw():
        ...
        display_vidas = fonte.render(f"\u2764{carro.vidas}", 1, (255, 0, 0))  # Renderiza o nome "Vidas" junto a sua respectiva variável
        display.blit(display_vidas, (10, 10))  # pos. onde display_vidas será mostrado na tela
        display_score = fonte.render(f'Score: {score}/99', 1, (255, 255, 255))
        display.blit(display_score, (10, 50))  # Mostra o score na tela
        
        if boss_spawnado:
            boss_hp_text = fonte.render(f'Cãopeta: {boss.hp}', 1, (255, 0, 0))  # Red color for boss health
    ...
    def game_over():
        pygame.quit()
        subprocess.run(["python", "py_jogoMenu.py"])
        sys.exit()
    ...
    while rodando:
        ...
        for inimigo in inimigos:
            ...
            # Checa colisão entre projéteis do player e inimigos
